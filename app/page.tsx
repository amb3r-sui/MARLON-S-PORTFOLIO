...
export default function Home() {
  return <>
    <Hero />
    <section className="section systems-section"><ScrollReveal className="container"><div className="section-top"><SectionHeading eyebrow="01 · Project lab" title="Choose a real system and test its business outcome" description="Each project opens with a client-friendly sample run, readable results, failure scenarios, and the complete audited workflow only when you choose to inspect it." /><Link href="/projects" className="text-link">Enter the full project lab <ArrowRight size={17} /></Link></div><div className="project-grid project-bento">{projects.filter((p) => p.featured).map((project) => <ProjectCard key={project.id} project={project} />)}</div></ScrollReveal></section>
