...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    <article>
      <header className="case-hero"><div className="container"><Link href="/projects" className="back-link"><ArrowLeft size={16} /> All projects</Link><div className="case-meta"><span>{project.category}</span><span>{project.projectType}</span></div><h1>{project.title}</h1><p>{project.summary}</p><div className="tag-row">{project.tools.map((tool) => <span className="tag" key={tool}>{tool}</span>)}</div><div className="implementation-state">{project.githubUrl ? <a className="text-link" href={project.githubUrl} target="_blank" rel="noopener noreferrer">View repository <ExternalLink size={16} /></a> : <span>Private implementation</span>}{project.demoMode === "simulation" ? <span>Interactive simulation available below</span> : project.demoUrl ? <a className="text-link" href={project.demoUrl} target="_blank" rel="noopener noreferrer">External demo <ExternalLink size={16} /></a> : <span>Demo not published</span>}</div></div></header>
      <div className="container case-layout">
        <main className="case-main">
          ...
          {project.demoMode === "simulation" && project.demoScenarios.length > 0 && <CaseSection eyebrow="Interactive case study" title="Test the business outcome"><p>Choose a fictional scenario, run the workflow, and review the selected route, safeguards, and client-friendly result. Technical JSON and the full n8n-style canvas remain available as optional details.</p><WorkflowDemo project={project} /></CaseSection>}
          ...
          <CaseSection eyebrow="Data contract" title="Inputs, outputs, and validation"><div className="three-card-grid"><div><h3>Inputs</h3><ul>{project.inputs.map((item) => <li key={item}>{item}</li>)}</ul></div><div><h3>Outputs</h3><ul>{project.outputs.map((item) => <li key={item}>{item}</li>)}</ul></div><div><h3>Validation rules</h3><ul>{project.validationRules.map((item) => <li key={item}>{item}</li>)}</ul></div></div></CaseSection>
          ...
          <CaseSection eyebrow="My contribution" title="What I designed and implemented"><div className="feature-list">{project.personalContribution.map((item) => <div key={item}><CheckCircle2 />{item}</div>)}</div></CaseSection>
          <CaseSection eyebrow="Scope" title="Current limitations and future improvements"><div className="two-card-grid"><div><h3>Current limitations</h3><ul>{project.limitations.map((item) => <li key={item}>{item}</li>)}</ul></div><div><h3>Future improvements</h3><ul>{project.futureImprovements.map((item) => <li key={item}>{item}</li>)}</ul></div></div></CaseSection>
          ...
          <CaseSection eyebrow="Expected operational impact" title="A more consistent, traceable process"><p>{project.businessImpact}</p><p className="honesty-line">This is an expected impact statement, not a claim of verified client results.</p></CaseSection>
        </main>
